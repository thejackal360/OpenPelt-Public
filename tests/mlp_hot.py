...
if __name__ == "__main__":
    if not os.path.exists('./results/'):
        os.mkdirs('./results/')
    pC = bessel.plant_circuit("Detector", None, bessel.Signal.VOLTAGE, plate_select = bessel.TECPlate.COLD_SIDE)
    cbs = bessel.circular_buffer_sequencer([50.00, 30.00, 40.00], pC.get_ncs())
    nc = bessel.neural_controller(cbs, hidden_units = 6, lrate = 0.5, plate_select = bessel.TECPlate.COLD_SIDE)
    pC.set_controller_f(nc.controller_f)
    pC.run_sim()
    pC.plot_th_tc(bessel.IndVar.TIME, plot_driver = True, include_ref = True)
